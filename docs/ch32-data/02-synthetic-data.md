...
The most common use of a model to make data is **distillation**: run a strong
model (the *teacher*) over a set of prompts, keep its outputs, and fine-tune a
smaller or cheaper model (the *student*) on them. The name and the original idea
come from Hinton, Vinyals and Dean's 2015 work on distilling the knowledge in a
neural network, where the student learned from the teacher's full output
distribution. In LLM practice the usual version is coarser and simpler: the
student is trained on the teacher's *sampled text*, exactly as if a human had
written it. Stanford's Alpaca is the best-known early public example — a small
open model fine-tuned on instruction data generated by a much stronger one.
...
Two technical limits are worth stating up front. A student trained on a
teacher's outputs inherits the teacher's mistakes *and* its ceiling: it is
learning to imitate a specific model, not to be correct. And it inherits style
strongly — distilled models often sound exactly like their teacher while being
noticeably worse at the reasoning underneath, because imitating surface form is
much easier than reproducing the computation.
...
The arrow back into the pool is what makes it *self*-instruct: accepted
instructions become few-shot examples for the next round, so the pool
bootstraps itself. The filter marked "too similar" is the load-bearing part.
Without it the model proposes the same handful of tasks forever, and you get a
large dataset with a tiny effective size.
...
substring, so it tolerates insertions and reordering. Here it is implemented and
...
tasks the pool already contained, and the similarity filter did all the work —
...
discarded if it gained no information over its parent, if the model cannot
answer it, if the answer is empty or nothing but stop words, or if the rewriting
prompt's own template text leaked into the instruction. Rewriting blindly is how
you get a dataset of unanswerable garbage, so the checks come with the method.
...
- **vocabulary size** and **type-token ratio**: how many different words the set
  uses at all.
...
and the spread shrinks geometrically. That is only half the story, though: a
Gaussian has no *categories* to lose. The block below therefore runs two
simulations side by side — a Gaussian chain to check the formula, and a
vocabulary chain over 300 words with a Zipf-like frequency curve to watch rare
items disappear. A word that draws zero samples in one generation has
probability exactly zero in the next fit, and can never come back. That is
extinction, and it is irreversible.
...
data in the mix. Accumulating rather than replacing holds the spread at 0.928
instead of 0.444. In practice this means anchoring every synthetic batch to
something real — a source document, a human seed, a verified answer — rather
than to the previous batch of synthetic data.
